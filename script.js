...
const stagePlanGrade1 = [
  {stage:"input", label:"Смотрим и узнаём"},
  {stage:"noticing", label:"Замечаем и различаем"},
  {stage:"controlled", label:"Выбираем по образцу"},
  {stage:"retrieval", label:"Вспоминаем с картинкой"},
  {stage:"controlled", label:"Закрепляем"},
  {stage:"context", label:"Ищем в ситуации"},
  {stage:"retrieval", label:"Вспоминаем без полного образца"},
  {stage:"personalization", label:"Выбираем про себя"},
  {stage:"speaking", label:"Говорим по опоре"},
  {stage:"production", label:"Финальная мини-миссия"}
];
...
function stageForPage(i, total, grade){
  const plan = grade===1 ? stagePlanGrade1 : stagePlan;
  if(total === 10) return plan[i];
  const pos = total<=1 ? 0 : i/(total-1);
  const idx = Math.min(plan.length-1, Math.round(pos*(plan.length-1)));
  return plan[idx];
}
...
function buildPlan(seed){
  ...
  for(let i=0;i<pages;i++){
    const stageObj=stageForPage(i,pages,grade);
...
function gradeProfile(g){
  return {
    1:"6–7 лет, Starter / самый ранний Pre-A1. Не предполагай уверенного чтения и письма. Главная опора — крупные картинки, действия, выбор, соединение, обводка, раскрашивание по условию и устное повторение. Письмо — максимум отдельные буквы, обводка или 1 знакомое слово, только если это уже есть на скриншотах.",
...
function gradeOneRules(g){
  if(String(g)!=="1") return "";
  return `
ОСОБЫЙ РЕЖИМ ДЛЯ 1 КЛАССА
— Максимально простые задания. Один шаг = одно действие.
— Не рассчитывай на самостоятельное чтение длинных инструкций. Инструкция ученику обычно 1–4 очень знакомых слова или понятная визуальная команда.
— Основные действия: look, point, match, circle, color, trace, choose, say, listen, find.
— Не используй длинные тексты, абзацы, письменные ответы предложениями, кроссворды с большим количеством букв, сложные wordsearch, объяснение грамматических правил или метаязык.
— Если выбран фокус Reading, трактуй его как раннее чтение: узнавание знакомого слова, первой буквы, короткой подписи или знакомой реплики. Связный текст добавляй только если ФОТО 1 и 2 явно показывают, что дети уже читают его.
— Если выбран фокус Grammar, не объясняй правило. Давай образец + 2–4 очень простых примера с картинками.
— Если выбран фокус Speaking, ответ может состоять из одного слова или одной знакомой модели.
— На странице лучше 2 основных задания. Если выбрано 3, третье делай микрозаданием на 1–2 минуты.
— Не требуй переписывать целые предложения.
— Крупные картинки и крупные поля важнее количества контента.
— В заданиях с выбором должно быть 2–3 варианта, не больше.
— Даже простое задание не должно решаться по расположению ответа: правильная картинка/реплика меняет позицию, а визуальные дистракторы остаются правдоподобными.
`;
}
...
ТВОЯ ЗАДАЧА
Создай полностью готовую к использованию рабочую тетрадь по постоянной системе входных изображений: фото 1 и фото 2 — учебный материал, фото 3 — визуальный мудборд.

ВАЖНО О ВХОДНЫХ ДАННЫХ
— У тебя всегда есть три входных изображения.
— ФОТО 1 = скриншот(ы) учебника.
— ФОТО 2 = скриншот(ы) учебника.
— ФОТО 3 = отдельный визуальный мудборд.
— ФОТО 1 и ФОТО 2 используй ТОЛЬКО как содержательный источник: тема, тематический контент, активная лексика, грамматические модели, речевые функции, типы текстов, знакомые формулировки и реальный уровень сложности.
— По ФОТО 1 и ФОТО 2 определи, какой языковой материал уже дан детям и насколько сложными должны быть задания.
— ФОТО 3 используй ТОЛЬКО как визуальный источник: стиль рисовки, палитра, композиционные приёмы, декоративные элементы, тип фонов и персонажей.
— ФОТО 1 и ФОТО 2 НЕ влияют на стиль оформления.
— ФОТО 3 НЕ влияет на выбор учебного содержания.
— Не проси меня перепечатывать материал со скриншотов.
— Не копируй дизайн учебника.
— НЕ используй персонажей из учебника.
— Все герои рабочей тетради должны быть взяты из ФОТО 3 и сохранять узнаваемый внешний вид от страницы к странице.
— Не придумывай отдельный «unit focus» и не сужай материал до искусственно выбранной подтемы. Используй весь совместимый языковой материал, реально представленный на ФОТО 1 и ФОТО 2.
...
— Постоянная система источников: ФОТО 1 + ФОТО 2 = содержание и уровень; ФОТО 3 = стиль и персонажи.
— Язык инструкций ученику: ${instr}.
— Визуальный стиль и персонажи: ${styleDescription()}
— ${expansionText(state.expansion)}
${gradeOneRules(grade)}${watermark ? `— На каждой странице аккуратно размести watermark: ${watermark}.` : ""}
...
9. Проверяй, чтобы хотя бы большинство заданий действительно требовали подумать на уровне возраста: сравнить картинки, вспомнить знакомое слово, соотнести смысл, заметить отличие или применить знакомый образец, а не просто ткнуть в первый похожий ответ. Для 1 класса мыслительное усилие должно быть коротким и посильным, без лишней нагрузки чтением.
...
— ФОТО 1 и ФОТО 2 определяют содержание. ФОТО 3 определяет визуал.
